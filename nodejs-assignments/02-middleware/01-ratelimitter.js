...
app.use((req, res , next) => {
    ...
    if(!userName){
      res.status(400).json({msg: 'userName is required in header'});
      return;
    }
    ...
    if(!numberOfRequestsForUser[userName]){
      numberOfRequestsForUser[userName] = {count: 0, LastReqTime: currentTime};
    }

    const userData = numberOfRequestsForUser[userName];
